...
# `virp`: VIRtual cell generation by Permutation
 ...
 3. Run `properties.py` to predict materials properties. This is performed on `stropt` subfolders only.
 ...
 `v2.1.x`: works on simple correlated disorder (water ice), site groups, and the P5 rule<br>
 `v2.2.x`: works on site networks (e.g. superionic materials)
